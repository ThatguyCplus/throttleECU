...
class ThrottleGUI:
    ...
    def _build_ui(self):
        ...
        # ══════════════════════════════════════
        # ── ERROR FLAGS PANEL (ASIL safety) ──
        # ══════════════════════════════════════
        err_frame = tk.Frame(left, bg=self.DARK_RED, highlightbackground=self.RED,
                              highlightthickness=0)
        err_frame.pack(fill="x", pady=(0, 6), ipady=3, ipadx=4)

        err_title_row = tk.Frame(err_frame, bg=self.DARK_RED)
        err_title_row.pack(fill="x", padx=4, pady=(2, 0))
        tk.Label(err_title_row, text="Safety Status", bg=self.DARK_RED,
                  fg=self.RED, font=("Segoe UI", 11, "bold")).pack(side="left")
        self.lbl_err_hex = tk.Label(err_title_row, text="0x00", bg=self.DARK_RED,
                                     fg=self.OVERLAY, font=("Consolas", 10))
        self.lbl_err_hex.pack(side="right")

        # Diagnostics button
        diag_row = tk.Frame(err_frame, bg=self.DARK_RED)
        diag_row.pack(fill="x", padx=4, pady=(0, 2))
        tk.Button(diag_row, text="DIAG", bg=self.OVERLAY, fg=self.FG,
                   font=("Consolas", 8), width=5, relief="flat",
        ...
        err_grid.pack(fill="x", padx=4, pady=2)
        for i, (bit, short, desc) in enumerate(ERR_NAMES):
            row = i // 4
            col = i % 4
            lbl = tk.Label(err_grid, text=short, bg=self.DARK_RED, fg=self.OVERLAY,
                            font=("Consolas", 8, "bold"), width=10, anchor="w")
            lbl.grid(row=row, column=col, padx=2, pady=1, sticky="w")
            self.err_indicators.append((bit, lbl))
    ...
    def _update_errors(self, flags):
        self.lbl_err_hex.config(text=f"0x{flags:02X}")
        any_active = flags != 0

        for bit, lbl in self.err_indicators:
            if flags & bit:
                lbl.config(fg=self.RED, bg="#5c1a1a")
            else:
                lbl.config(fg=self.OVERLAY, bg=self.DARK_RED)

        # Flash the error panel border if any error active
        if any_active:
            # Highlight border
            pass  # handled by label colors
